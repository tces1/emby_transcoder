...
const stateLabel={downloading:'下载中',forwarding:'转发中',probing:'探测中',running:'转码中',idle:'空闲',disabled:'未启用',paused:'已暂停',error:'错误',exited:'已结束'};
const label=s=>stateLabel[s]||s;
const hist=new Map();
...
function render(data){
 ...
 document.getElementById('workers').innerHTML=(data.workers||[]).map(function(w){return '<div class="card"><div class="worker-head"><b>Worker '+esc(w.id)+'</b><span class="badge '+badge(w.state)+'">'+esc(label(w.state))+'</span></div><div class="metric">'+rate(w.download_bps)+'</div><div class="muted">累计 '+bytes(w.total_bytes)+'</div><div class="route">'+esc(w.route||'等待任务')+' · '+esc(w.byte_range||'-')+'</div><div class="muted">'+esc(w.video_name||'暂无视频')+'</div></div>'}).join('');
