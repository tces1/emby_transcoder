...
启动下载时只并行探测优先级最高的两条线路；取得两个可用且内容一致的最终媒体响应后立即停止，不会扫描后续入口。只有缺少第二条可用线路时才按顺序探测下一条。最终媒体域名相同的入口会去重；线路通过 ETag/Last-Modified，或文件大小加头部、中间、尾部三段 SHA-256 指纹确认内容一致。
